...
# 先装依赖，利用层缓存（postinstall 需要 patch 脚本，故一并拷贝 scripts/）
...
#     混入系统驱动反而可能因版本不匹配 dlopen 失败。
#   · 发行版 libva2：bookworm 只有 2.17.0，缺 vaMapBuffer2 符号，
#     而 jellyfin-ffmpeg 需要 ≥2.21 —— 被误加载会直接 abort
#     （历史事故记录：曾误删自带 libva 改用系统 2.17.0，正是当时的根因）。
#
# 部分构建环境无法直连 deb.debian.org，切到国内镜像源以保证可拉包。
# 注意：必须用 HTTPS 拉取（http 端口 80 在本环境不可达，会连不上 Packages 索引）。
...
# 这是"换设备也能用"的关键，改动前请先读 Dockerfile.nas 里同一段说明并实测。
