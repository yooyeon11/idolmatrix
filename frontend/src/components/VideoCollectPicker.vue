...
const emit = defineEmits<{
  change: [videoId: number, collectionIds: number[]]
  removedFromCurrent: []
}>()
...
async function load() {
  ...
  try {
    const [cols, ms] = await Promise.all([
      ...
      videoCollectionsApi.memberships([props.video.id]),
    ])
    collections.value = cols
    const ids = ms.memberships[String(props.video.id)] || ms.memberships[props.video.id as unknown as string] || []
    selected.value = new Set(ids)
    emit('change', props.video.id, ids)
  } catch (e) {
...
async function toggle(col: VideoCollection) {
  savingId.value = col.id
  const next = new Set(selected.value)
  try {
    if (next.has(col.id)) {
      await videoCollectionsApi.removeVideo(col.id, props.video.id)
      next.delete(col.id)
      if (props.currentCollectionId === col.id) emit('removedFromCurrent')
    } else {
      await videoCollectionsApi.add(col.id, { video_uid: props.video.uid, video_id: props.video.id })
      next.add(col.id)
    }
    selected.value = next
    emit('change', props.video.id, [...next])
    collections.value = await videoCollectionsApi.list()
  } catch (e) {
    message.error((e as Error).message)
  } finally {
    savingId.value = null
...
<template>
  <div class="collect-picker" @click.stop @pointerdown.stop>
    ...
    <div v-else-if="!collections.length" class="collect-empty">还没有视频收藏夹</div>
    <div v-else class="collect-list">
      ...
      >
        ...
        <span class="collect-count">{{ col.video_count }}</span>
      </button>
    </div>
